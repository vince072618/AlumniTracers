...
const YearSelect: React.FC<YearSelectProps> = ({ value = '', onChange, placeholder = 'Select year', className = '' }) => {
  ...
  const years = React.useMemo(() => Array.from({ length: 2030 - 2009 + 1 }, (_, i) => 2030 - i), []);
          ...
          {years.map(y => (
            <li key={y}>
              <button
                ...
                className={`w-full text-left px-4 py-2 hover:bg-gray-100 ${value === y ? 'bg-blue-600 text-white' : 'text-gray-900'}`}
              >
                {y}
              </button>
            </li>
